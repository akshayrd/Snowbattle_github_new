...
-(id) initWithPlayer:(BOOL)player1 timeBonus:(int) timeRemaining powerups:(int [])powerupArray playerSelected:(int [])playerSelectArray
    {
        if( (self=[super init]) ) {
            ...
            if(powerupArray[2]>=1)
            {
                ...
                shopPowerUp2 = [CCMenuItemImage itemFromNormalImage:@"powerUp_ghostPitClose.png" selectedImage:@"powerUp_ghostPitClose.png" target:self selector:@selector(Closepit:)];
                //shopPowerUp2.position = ccp(22, 600-10*40);
                //shopPowerUp2.visible = true;
                
                Powerupmenu2 = [CCMenu menuWithItems: shopPowerUp2, nil];
                Powerupmenu2.position=ccp(22,600-10*40);
                Powerupmenu2.visible = true;
                //[menu alignItemsVerticallyWithPadding:15];
                [self addChild:Powerupmenu2];
            }
